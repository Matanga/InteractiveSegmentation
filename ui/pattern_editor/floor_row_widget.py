...
class FloorRowWidget(QWidget):
    ...
    This widget composes a FloorHeaderWidget and four FacadeCellWidgets.
    """
    ...
    def __init__(self, floor_idx: int, mode: str = REPEATABLE, parent: QWidget | None = None):
        ...
        self.header.set_initial_label(self.floor_index)
    ...
    def get_floor_data(self) -> Dict[str, Any]:
        ...
        for cell in self.facade_cells:
            ...
            for j in range(cell.module_container_layout.count()):
                ...
                module_names = [mod.name for k in range(group.layout().count()) if isinstance(mod := group.layout().itemAt(k).widget(), ModuleWidget)]
        ...
        floor_data = {"Name": self.header.name_edit.text(), "Pattern": pattern_array, "Height": int(self.header.height_edit.text() or 0)}
        return floor_data
