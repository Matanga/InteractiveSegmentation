...
class SegmentationPanel(QtWidgets.QWidget):
    """3-step *image → symbolic → rigid → repeatable* pipeline panel."""
    ...
    def _build_ui(self) -> None:
        """
        Assemble the widgets and layouts for the EndpointPanel in a three-column layout.

        This method constructs the UI step-by-step, following a clear hierarchy.
        ...
        # --- Widgets for Column 1 (Actions) ---
        style = self.style()
        self.btn_sym = QtWidgets.QPushButton("1. Generate Symbolic")
        self.btn_sym.setIcon(style.standardIcon(QtWidgets.QStyle.StandardPixmap.SP_MediaPlay))
        self.btn_sym.setToolTip("Generate the symbolic representation from the current pattern\nRequires Input Image")

        self.btn_rigid = QtWidgets.QPushButton("2. Generate Rigid")
        self.btn_rigid.setIcon(style.standardIcon(QtWidgets.QStyle.StandardPixmap.SP_MediaPlay))
        self.btn_rep = QtWidgets.QPushButton("3. Generate Repeatable")
        self.btn_rep.setIcon(style.standardIcon(QtWidgets.QStyle.StandardPixmap.SP_MediaPlay))
        self.btn_send_to_editor = QtWidgets.QPushButton("➤ Send to Editor")
        self.btn_send_to_editor.setIcon(style.standardIcon(QtWidgets.QStyle.StandardPixmap.SP_ArrowRight))
        self.btn_send_to_editor.setObjectName("SendToEditor")  # Set object name for styling

        # --- Widgets for Column 2 (Input & Params) ---
        self.in_label = ImageDropLabel("Click or drop an image")
        self.params_box = self._build_params_box()

        # --- Widgets for Column 3 (Outputs) ---
        ...
        # --- Widgets for the Bottom Status Row ---
        self.progress = QtWidgets.QProgressBar(textVisible=False)
        self.status = QtWidgets.QLabel("Please load an image to begin.")

        # =======================================================================
        #  STEP 2: ASSEMBLE EACH COLUMN
        # =======================================================================

        # --- Assemble Column 1: Workflow Actions ---
        # <<< FIX: This section is corrected to properly contain the GroupBox.
        actions_box = QtWidgets.QGroupBox("Workflow Actions")
        actions_box.setObjectName("WorkflowActions")
        actions_lay = QtWidgets.QVBoxLayout(actions_box)  # This is the *internal* layout for the box
        ...
        # Now, create the column's main layout and add the entire group box to it.
        actions_column_layout = QtWidgets.QVBoxLayout()
        actions_column_layout.addWidget(actions_box)
        actions_column_layout.addStretch(1)  # Push the group box to the top

        actions_column_widget = QtWidgets.QWidget()
        actions_column_widget.setLayout(actions_column_layout)  # Set the layout for the column container
        actions_column_widget.setMinimumWidth(220)

        # --- Assemble Column 2: Input & Parameters ---
        ...
        input_params_layout.addWidget(input_box, 1)  # Give stretch factor
        ...
        # --- Assemble Column 3: Visual & Text Outputs ---
        ...
        # --- Assemble the Content Splitter (for columns 2 and 3) ---
        ...
        # --- Assemble the Root Splitter (for column 1 and the rest) ---
    ...
    def _make_label(text: str) -> QtWidgets.QLabel:
        """Return a framed placeholder label."""
    ...
    def _build_params_box(self) -> QtWidgets.QGroupBox:
        """Create the parameters form (unchanged semantic)."""
    ...
    def on_image_loaded(self, path: str) -> None:
        """Handle new image drop / selection."""
    ...
    def start_symbolic(self) -> None:
        """Kick off *Symbolic* generation thread."""
    ...
    def start_rigid(self) -> None:
        """Kick off *Rigid* generation thread."""
    ...
    def start_repeatable(self) -> None:
        """Kick off *Repeatable* generation thread."""
    ...
    def _run_thread(
        ...
        done_slot: QtCore.Slot,
        status_msg: str,
    ) -> None:
        """Common helper for launching worker threads."""
    ...
    def _on_thread_finished(self) -> None:
        """Always reset thread ref + UI after worker exits."""
    ...
    def _set_busy_state(self, message: str) -> None:
        """Disable buttons and show busy indicator."""
    ...
    def _update_ui_state(self) -> None:
        """Enable/disable controls based on current stage + thread state."""
        self.progress.hide()
        ...
        self.btn_send_to_editor.setEnabled(
            is_idle and self._final_repeatable_text is not None
        )

    # ------------------------------------------------------------------ #
    def _clear_outputs(self) -> None:
        """Reset all derived data and UI placeholders."""
        for lbl, txt in (
    ...
    def _set_label_pixmap_from_data(label: QtWidgets.QLabel, data: bytes) -> None:
        """Show *data* (PNG/JPG) in *label* scaled to fit."""
    ...
    def _cfg(self) -> dict[str, Any]:
        """Collect current parameter-form values -> dict consumed by RigidThread."""
